...
//--------------------------------------------------------
long StreamReassembler::merg_block(block_node& elm1,const block_node& elm2) {
    block_node x,y;
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    if(index >= _head_index + _capacity) return; //capacity over
    ...
    do{
        ...
        while(iter != _blocks.end() && (merged_bytes = merg_block(elm, *iter)) >= 0) {
        ...
        iter--;
        while((merged_bytes = merg_block(elm, *iter)) >= 0) {
            _unassembled_byte -= merged_bytes;
            _blocks.erase(iter);
            iter = _blocks.lower_bound(elm);
    ...
    if(!_blocks.empty() && _blocks.begin()->begin == _head_index) {
        ...
        //modify _head_index and _unassembled_byte according to successful write to output
